...
| `finalize [<sid>] [--no-apply]` | End the current session now and consolidate it inline — the pre-PR flush: wiki pages land in the working tree while the feature branch is still checked out, so they ride in the same PR instead of trailing the merge. Applies without review (an explicit command, like `commit`); `--no-apply` keeps the proposal for review. The chat can keep going — later events open a fresh incarnation |
...
| `memoria_consolidate` | Batch-consolidate ended sessions; when the proposal is ready the agent reviews the page list and calls again with `apply=true`. `end_current=true` first marks the caller's still-open session ended — the pre-PR flush (`memoria finalize` from the CLI) |
...
- **Pending queue** — new digests are registered in `~/.config/memoria/pending.yaml`, grouped by project. A session counts as ended when `session-end` fires, when a new session starts in the same project (so crashed sessions don't linger), or when you end it early with `memoria finalize` / `memoria_consolidate end_current=true`; `memoria process` consumes this worklist. Agents also get a nudge: `gh pr create` from a session with unflushed observations returns hook context suggesting the flush, so the wiki lands on the feature branch before the PR merges.
