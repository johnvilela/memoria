...
memoria process            # LLM proposes pages -> .memoria/proposal.json
...
|---------|-------------|
| `help` | Show usage |
| `init [<client>] [--client ...] [--processor ...]` | Install memoria hooks for an agent (claude-code \| codex) and choose the AI provider that processes sessions (claude-code \| codex \| ollama \| gemini); interactive when flags are omitted |
| `bootstrap` | Register the current folder as a tracked project; gitignores `.memoria/` and creates the wiki folder |
| `process [--apply]` | Consolidate ended pending sessions into the wiki: writes a JSON proposal for review, `--apply` creates the files |
...
Config lives at `~/.config/memoria/config.yaml` (`projects`, `processor`, and `gemini_api_key` when the gemini processor is chosen — the file is written 0600). The gemini key can also come from the `GEMINI_API_KEY` env var.
