...
memoria run codex          # launch an agent continuing the last session
```
...
|---------|-------------|
| `help` | Show usage |
| `init [<client>] [--client ...] [--processor ...] [--notification] [--cron [expr]] [--cron-apply]` | Install memoria hooks for an agent (claude-code \| codex), register the memoria MCP server (claude-code: `~/.claude.json`, codex: `~/.codex/config.toml`), choose the AI provider that processes sessions (claude-code \| codex \| ollama \| gemini), opt into desktop notifications and the periodic systemd timer; interactive when flags are omitted |
| `setup [--processor ...] [--notification] [--cron <expr\|off>] [--cron-apply]` | Reconfigure processor / notifications / schedule without touching hooks; no flags = interactive walk over current values |
...
| `run <agent-binary> [--new \| --session <id\|name> \| --last-session]` | Launch any agent on PATH inside the project, continuing a previous session: same harness → native resume (`claude --resume`, `codex resume`), different harness → a handoff prompt pointing at the session digest |
...
Config lives at `~/.config/memoria/config.yaml` (`projects`, `processor`, `notifications`, `cron` and `gemini_api_key` when the gemini processor is chosen — the file is written 0600). The gemini key can also come from the `GEMINI_API_KEY` env var.
