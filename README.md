...
| `run <agent-binary> [--new \| --session <id\|name>]` | Launch any agent on PATH inside the project, continuing a previous session: same harness → native resume (`claude --resume`, `codex resume`), different harness → a handoff prompt pointing at the session digest. No flags → interactive picker: new session or one of the last 5 (sessions without a digest resume natively and may be slow) |
