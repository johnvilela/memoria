...
Semver, pre-1.0: minor bump per feature batch, patch bump per fix-only or tooling-only release. Version lives in **one place**: `const version` in `cmd/main.go` — the MCP handshake (`cmd/mcp.go`) and the `version` subcommand both read it.
...
2. Edit `const version = "X.Y.Z"` in `cmd/main.go`, in the same PR as the change that needs it.
3. Verify: `scripts/test.sh` (now vitest-style output via gotestsum's `testdox` format — see [[concepts/ci-release-pipeline]]), then `go run ./cmd version` prints the new number.
