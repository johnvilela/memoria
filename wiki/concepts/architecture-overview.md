...
**macOS config path differs**: Go's `os.UserConfigDir()` resolves the config/state directory to `~/Library/Application Support/memoria/` on macOS instead of `~/.config/memoria/`, unless `$XDG_CONFIG_HOME` is set (rare on a Mac). Same files, same layout — `config.yaml`, `memoria.log`, `status.yaml`, `pending.yaml`, `<project>.run.log` — just a different root. Confirmed while helping debug a Mac user's install; the user-level `~/Library/Application Support` is hidden by Finder by default and easy to confuse with the unrelated system-level `/Library/Application Support` ([[sessions/b83a5def-41be-43be-82b5-902466d6a5e6]]).