...
This forced-commit / never-commit split is implemented by `globalCommitCfg`, which flips `WikiAutoCommit` on a copy of the config at its call sites (`runProcess`, `processAll`, `runLint`, `resolveWorkspace` — né `mcpProject`, `runDigest`) rather than threading extra state through `commitWiki`.
...
Touched: `captureHook` (the capture gate), `process` (including `--all`, so the cron sweeps `_global` for free), `lint`, `mcp` (since v0.10.1 all seven tools resolve via `resolveProject`, so an MCP server launched in an unregistered folder falls back to the global workspace — sessions, wiki, page validation and commit policy included), `digest` (spawned by MCP `memoria_digest` with the caller's cwd, so its child must resolve the same way), and `search` (since v0.10.2 — same `resolveWorkspace` helper as the MCP tools, plus a headless mode: non-TTY multi-hit prints the ranked path list instead of the picker, so shell-only agents can search the global memory; since v0.13.0 leading `@_global` / `@all` query selectors reach the global wiki from any folder, no global fallback needed). With an empty `session_id`, "most recent session" means the newest *global* session, which may come from a different source folder than the caller's cwd — same pooling as the queue. Left project-scoped on purpose: `run`, `commit`, `seed` — human-facing resume/commit machinery.
